...
# Load hastag image for concatenate purpose
# TAG = cv2.imread('../Data/Image/htag.jpg')
        ...
        if detecteds:
            ...
            for detection in vehicle_IDs:
                ...
                if int(class_id) in VEHICLE_INDEX:
                    ...
                    # Draw Bounding boxes for each car in Detected List
                    cv2.rectangle(display_frame, (int(x1), int(y1)), (int(x2), int(y2)),  colorBlue, 3)
            ...
            for license_plate in license_plates.boxes.data.tolist():
                ...
                # Draw Bounding boxes for each car in Detected List
                cv2.rectangle(display_frame, (int(x1), int(y1)), (int(x2), int(y2)),  colorGreen, 3)

                # Add Car ID to the Car BBox and License plate BBox
                cv2.putText(display_frame, f'{score}', (max(0, int(x1)), max(40, int(y1))), cv2.FONT_HERSHEY_PLAIN, 3, colorGreen, 3)
