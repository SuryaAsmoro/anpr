...
def get_car(license_plate, vehicle_IDs):
    ...
    for i in range(len(vehicle_IDs)):
        xcar1, ycar1, xcar2, ycar2, car_id, cartype = vehicle_IDs[i]
...
def tracked_id_checker(tracked_IDs, vehicle_IDs, temp_results, frame_loss_tolerance = 3):
    ...
    for vehicle_ID in vehicle_IDs:
        ...
        xcar1, ycar1, xcar2, ycar2, car_id, cartype = vehicle_ID
            ...
            if vehicle_ID in temp_results[frame_nmr]:
                if 'car' in temp_results[frame_nmr][vehicle_ID].keys() and \
                    ...
                    if temp_results[frame_nmr][vehicle_ID]['license_plate']['text_score'] > best_text_score:
                        ...
                        best_cartype = temp_results[frame_nmr][vehicle_ID]['car']['type']
            ...
            if results.keys():
                ...
                while i <= merging_range:
                    ...
                    if i >= merging_range:
                        ...
                        # add current data        
                        results[vehicle_ID] = {'text': best_text, 'text_score': best_text_score, 'color': best_color, 'type': best_cartype}
...
MODEL_PATH = '../Models/Yolo-Weights/nanda/new/best.pt'
OUTPUT_PATH = '../Data/playground'

# Class IDs
plate_class_id = 2
vehicle_class_id = [0, 1, 3, 4]
...
while ret:
    ...
    if ret:
        ...
        for detection in detections.boxes.data.tolist():
            ...
            for vehicle_id in vehicle_class_id:

                if int(class_id) == vehicle_id:
                    ...
                    # Add the vehicle bounding box to the 
                    vehicle_detecteds.append([[[x1, y1, x2, y2, score]], cartype])

            if int(class_id) == plate_class_id:
        ...
        if vehicle_detecteds:
            ...
            for vehicle_bbox in vehicle_detecteds:
                print(f'Vehicle Bbox: {vehicle_bbox}')
                
                vehicle_bboxes += vehicle_bbox[0]

            print(f'Vehicle Bbox: {vehicle_bbox}')
            # Track Vehicles using SORT
            set_start_pin('Tracker')
            vehicle_IDs = motion_tracker.update(np.asarray(vehicle_bboxes))
            set_end_pin('Tracker')

            

            vehicle_ID_type_temp = []
            # Assign vehicle ID to vehicle data
            for i in range(len(vehicle_IDs)):
                xcar1, ycar1, xcar2, ycar2, car_id = vehicle_IDs[i]

                for i in range(len(vehicle_detecteds)):

                    print(f'vehicle Detecteds i : {vehicle_detecteds[i]}')
                    x1, y1, x2, y2, score = vehicle_detecteds[i][0][0]
                    cartype_ = vehicle_detecteds[i][1]
                    treshold = 10

                    if abs(x1 - xcar1) < treshold and abs(y1 - ycar1) < treshold and abs(x2 - xcar2) < treshold and abs(y2 - ycar2) < treshold:
                        
                        # create new dictionary identical to vehicle_ID but add cartype
                        vehicle_ID_type_temp += [[xcar1, ycar1, xcar2, ycar2, car_id, cartype_]]


            vehicle_IDs = vehicle_ID_type_temp
            ...
            for license_plate in plate_detecteds:
                ...
                if score > 0.4:
                    ...
                    xcar1, ycar1, xcar2, ycar2, car_id, cartype = get_car(license_plate, vehicle_IDs)
            ...
            if vehicle_detecteds:
                ...
                for detection in vehicle_IDs:
                    ...
                    # Extract BBox and Car ID data from Vehicle_ID
                    x1, y1, x2, y2, car_id, cartype = detection
